...
impl IntegrityRootStore for VolatileFixtureRoot {
    fn load(&self) -> Result<Option<IntegrityCheckpoint>, IntegrityRootError> {
        Ok(self.0.lock().map_err(|_| IntegrityRootError::Unavailable)?.clone())
    }
    fn compare_and_store(&self, expected: Option<&IntegrityCheckpoint>, next: &IntegrityCheckpoint) -> Result<(), IntegrityRootError> {
        ...
        if stored.as_ref() != expected { return Err(IntegrityRootError::Conflict); }
...
impl BrokerJournalClock for FixtureClock {
    fn now_unix_seconds(&self) -> Result<i64, BrokerAuditError> { Ok(110) }
}
impl VaultBrokerClock for FixtureClock {
    fn now_unix_seconds(&self) -> Result<i64, BackendDispatchError> { Ok(110) }
}
struct SyntheticTransport;
impl VaultBrokerTransport for SyntheticTransport {
    fn execute_openai_responses(&self, secret: &str, _: &OpenAiResponsesOperation, fence: &CancellationFence) -> Result<TypedOperationReceipt, BackendDispatchError> {
        fence.ensure_active()?;
        if secret != SYNTHETIC_SECRET { return Err(BackendDispatchError::Failed); }
        // No external network or model output; only a secret-free fixed receipt.
        Ok(TypedOperationReceipt { response_status: 204, uploaded_bytes: 0, downloaded_bytes: 0 })
    }
}
...
pub fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let peer = ObservedPeer::from_os(format!("uid:{}", nix::unistd::getuid()), u32::try_from(nix::unistd::getppid().as_raw())?)?;
    ...
    let vault = Arc::new(Vault::new_with_keyring_store(directory.path().join("vault"), Arc::new(MockKeyringStore::default())));
    vault.add(AddCredential { label: "synthetic-key".into(), credential_type: CredentialType::BearerToken, provider: Some("openai".into()), notes: None, revocation_notes: None, secret: SYNTHETIC_SECRET.into() })?;
    let text = |s: &str| BoundedText::new(s);
    let human = PolicyPrincipal::new(PrincipalKind::Human, "fixture-human")?;
    let actors = ActorChain::new(vec![human.clone(), PolicyPrincipal::new(PrincipalKind::Agent, "fixture-agent")?])?;
    let authorization = AuthorizationRequest::new(actors.clone(), ProtectedResource::new(ResourceKind::VaultCredential, "synthetic-key")?, PolicyAction::Use, AuthorizationContext {
        now_unix_seconds: 100, session_id: text("session")?, task_id: text("task")?, purpose: text("fixture")?, operation: text("openai.responses.create")?, destination: Some(text("https://api.openai.com:443")?), quantity: None, grant_id: None,
    })?;
    let grant = BoundedGrant::issue(human, actors, GrantScope::new(authorization.resource.clone(), [PolicyAction::Use], GrantContext::new(authorization.context.session_id.clone(), authorization.context.task_id.clone(), authorization.context.purpose.clone(), authorization.context.operation.clone()), authorization.context.destination.clone(), BTreeMap::new())?, 90, 200, text("fixture-grant")?)?;
    let revocations = RevocationState::new();
    let request = CredentialCapabilityRequest::new(authorization, grant, codex_security_policy::CredentialReference::new("synthetic-key", "openai.responses.create")?, CredentialHttpMethod::Post, CredentialDestination::https("api.openai.com", 443)?, "/v1/responses", 100, 180, &revocations, None)?;
    let reference = codex_secret_broker::CredentialReference::from_sha256_hex("a".repeat(64))?;
    let credential = VaultCredentialRef::from_authorized(CapabilityId::from_sha256_hex(reference.as_str())?, request.clone())?;
    let operation = OpenAiResponsesOperation::new("/v1/responses")?;
    let binding = BrokerBinding { controller_instance: "controller".into(), worker_instance: "worker".into(), session_id: "session".into(), task_id: "task".into(), run_id: "run".into(), run_generation: 1 };
    ...
    let count = root.load()?.map(|checkpoint| checkpoint.sequence).unwrap_or(0);
    println!("synthetic-only journal-records={count}");
    if !matches!(result, Err(BrokerDispatchError::SessionUnavailable)) { result?; }
    Ok(())
}

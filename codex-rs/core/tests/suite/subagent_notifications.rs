...
async fn wait_for_input_type(
    ...
    loop {
        if let Some(request) = mock
            .requests()
            .into_iter()
            .find(|request| !request.inputs_of_type(expected_type).is_empty())
        {
            return Ok(request);
        }
        if Instant::now() >= deadline {
            anyhow::bail!("expected a request containing input type `{expected_type}`");
        }
        sleep(Duration::from_millis(10)).await;
    }
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn provider_neutral_multi_agent_v2_spawn_sends_agent_message_to_child() -> Result<()> {
    let server = start_mock_server().await;
    ...
    let child_request = wait_for_input_type(&child_request_log, "agent_message").await?;
    assert_eq!(

...
async fn accounting_chat_native_off_and_mode_isolation() -> anyhow::Result<()> {
    for index in 0..5 {
        ...
        let mode = match index {
            ...
            2 => AccountingMode::DirectAnthropic { scope: uuid::Uuid::new_v4(), approved_endpoint: endpoint.clone() },
            3 => AccountingMode::DirectOpenAiResponsesHttp { scope: uuid::Uuid::new_v4(), approved_endpoint: endpoint.clone() },
            _ => AccountingMode::DirectOpenAiResponses { scope: uuid::Uuid::new_v4(), approved_endpoint: endpoint.clone() },
        };
        let test = builder(endpoint, mode).with_config(move |c| {
            if index == 1 { c.features.disable(codex_features::Feature::Sqlite).unwrap(); }
        }).build_with_auto_env(&server).await?;
        test.submit_turn("fixture").await?;
        posts(&server, 1).await;
        if let Some(db) = test.codex.state_db() { absent(&db).await?; }
        stop(&test).await;
    }
    for wire in [codex_model_provider_info::WireApi::Responses, codex_model_provider_info::WireApi::Anthropic] {
        let server = MockServer::start().await;
        let endpoint = format!("{}/v1", server.uri());
        ...
        let test = builder(endpoint.clone(), enabled(&endpoint))
            .with_config(move |c| c.model_provider.wire_api = wire)
            .build_with_auto_env(&server).await?;
        submit(&test).await?;
        terminal(&test).await?;
...
async fn accounting_chat_native_literal_partial_and_zero_goldens() -> anyhow::Result<()> {
    for (cached, zero) in [(true,false),(false,false),(true,true)] {
        ...
        let endpoint = format!("{}/v1", server.uri());
        let mut raw = usage();
        if !cached { raw.as_object_mut().unwrap().remove("prompt_tokens_details"); }
        if zero { raw = json!({"prompt_tokens":0,"completion_tokens":0,"total_tokens":0,
...
async fn accounting_chat_native_cumulative_and_separate_usage() -> anyhow::Result<()> {
    let server = MockServer::start().await;
    let endpoint = format!("{}/v1", server.uri());
    mount(&server, event(usage()) + ": comment\n\n" + &data(json!({"choices":[]}))
        + &event(usage()) + &data(json!({"choices":[]}))
        + &event(json!({"prompt_tokens":null})) + &ending("stop")).await;
    let test = builder(endpoint.clone(), enabled(&endpoint)).build_with_auto_env(&server).await?;
    test.submit_turn("fixture").await?;
    posts(&server, 1).await;
    let db = test.codex.state_db().unwrap();
    ...
    let patches = observations(&db).await?;
    assert_eq!(patches.iter().map(|p| i64::from(p.sequence)).collect::<Vec<_>>(), vec![1,3,5]);
...
async fn accounting_chat_native_top_level_error_retains_usage() -> anyhow::Result<()> {
    for sibling in [true,false] {
        let server = MockServer::start().await;
        let endpoint = format!("{}/v1", server.uri());
        let payload = if sibling { data(json!({"error":{"message":"fixture terminal"},"usage":usage()})) }
            else { event(usage()) + &data(json!({"error":{"message":"fixture terminal"}})) };
...
async fn accounting_chat_native_immutable_prices_and_unpriced_rows() -> anyhow::Result<()> {
    for model in ["gpt-5.6-sol","gpt-6-astra","remote-only-fixture"] {
        ...
        let test = builder(endpoint.clone(), mode.clone()).with_config(move |c| c.model = Some(model.into()))
            .build_with_auto_env(&server).await?;
        test.submit_turn_with_service_tier("fixture",Some("priority")).await?;
        ...
        assert_eq!(before.known_usd, if model == "gpt-5.6-sol" {"0.00121"} else {"0"}.to_string().try_into()?);
        ...
        for index in 0..2 {
            let reopened = builder(endpoint.clone(),mode.clone())
                ...
                .resume(&server,home.clone(),rollout.clone()).await?;
            let db = reopened.codex.state_db().unwrap();
            assert_eq!(attempts(&db).await?,rows);
            assert_eq!(payloads::<Snapshot>(&db,"draft_accounting_price_snapshots").await?,prices);
            assert_eq!(totals(&db,&rows[0]).await?,before);
            assert_eq!(server.received_requests().await.unwrap().len(),1);
            if index == 1 {
                reopened.submit_turn("fresh").await?;
                assert_eq!(attempts(&db).await?.len(),2);
                assert_eq!(payloads::<Snapshot>(&db,"draft_accounting_price_snapshots").await?[..prices.len()], prices);
            }
            stop(&reopened).await;
            drop(reopened);
            db.close().await;
...
async fn accounting_chat_native_missing_usage_and_correlation() -> anyhow::Result<()> {
    for payload in [ending("stop"),success(Value::Null),data(json!({"id":"reused-provider-id","choices":[]}))] {
        let server = MockServer::start().await;
        let endpoint = format!("{}/v1",server.uri());
        mount(&server,payload).await;
        let test = builder(endpoint.clone(),enabled(&endpoint)).build_with_auto_env(&server).await?;
        for _ in 0..2 { submit(&test).await?; terminal(&test).await?; }
        ...
        assert_eq!(totals(&db,&rows[0]).await?,DayTotals {
            measured: std::array::from_fn(|_| Metric {known:0,unknown:2}),
            known_usd:"0".to_string().try_into()?,unknown_estimates:2,attempts:2,
        });
        stop(&test).await;
    }
    Ok(())
...
async fn accounting_chat_native_finish_reason_and_tool_parity() -> anyhow::Result<()> {
    for reason in ["stop","length","tool_calls","error"] {
        ...
        let test = builder(gate.endpoint.clone(),enabled(&gate.endpoint))
            .with_config(|c| { c.features.disable(codex_features::Feature::CodeModeOnly).unwrap(); })
            .build_with_auto_env(&server).await?;
        submit(&test).await?;
        let held = gate.next().await?;
        let payload = if reason == "tool_calls" {
            data(json!({"choices":[{"index":0,"delta":{"tool_calls":[{"index":0,"id":"safe-call",
                "type":"function","function":{"name":"shell_command","arguments":"{\"command\":\"echo fixture\"}"}}]},
                "finish_reason":"tool_calls"}],"usage":usage()})) + "data: [DONE]\n\n"
        } else {event(usage()) + &ending(reason)};
        held.chunks.send(payload).await?;
        let expected = if matches!(reason,"length"|"tool_calls") {
        ...
        if expected == 2 { assert_ne!(rows[0].request_id,rows[1].request_id); }
        gate.no_pending();
        stop(&test).await;
    }
    Ok(())
...
async fn accounting_chat_native_sampling_auxiliary_scope() -> anyhow::Result<()> {
    ...
    let test = builder(gate.endpoint.clone(),enabled(&gate.endpoint))
        .with_config(|c| {
            c.features.disable(codex_features::Feature::TokenBudget).unwrap();
            c.features.disable(codex_features::Feature::RemoteCompactionV2).unwrap();
        }).build_with_auto_env(&server).await?;
    submit(&test).await?;
    gate.next().await?.chunks.send(event(usage()) + &ending("length")).await?;
    gate.next().await?.chunks.send(success(usage())).await?;
    terminal(&test).await?;
    let db = test.codex.state_db().unwrap();
    let rows = attempts(&db).await?;
    ...
    for retry in [false,true] {
        ...
        if retry {
            Mock::given(method("POST")).respond_with(|request: &wiremock::Request| {
                ResponseTemplate::new(503).insert_header("x-corbanu-request-state","released")
                    .insert_header("x-corbanu-request-id",
                        request.headers.get("x-pfterminal-request-id").unwrap().to_str().unwrap())
            }).up_to_n_times(1).with_priority(1).mount(&server).await;
        }
        mount(&server,success(usage())).await;
        let gateway_endpoint = endpoint.clone();
        let test = builder(endpoint.clone(),enabled(&endpoint)).with_config(move |c| {
            let mut provider = codex_model_provider_info::ModelProviderInfo::create_pfterminal_plan_provider();
        ...
        let ids: Vec<_> = requests.iter().map(|r| r.headers.get("x-pfterminal-request-id").unwrap()).collect();
        if retry { assert_ne!(ids[0],ids[1]); }
        absent(&test.codex.state_db().unwrap()).await?;
        stop(&test).await;
    }
    Ok(())

...
#[test]
fn action_completion_guard_requires_structured_decision_after_tool_work() {
    let mut guard = ActionCompletionGuard::default();

    assert_eq!(
        guard.observe(
            /*chat_wire_api*/ true, /*saw_tool_call*/ true,
            /*needs_follow_up*/ true, /*explicit_completion*/ false,
        ),
        CompletionGuardDecision::ContinueNormally
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        CompletionGuardDecision::Accept
    );
}

#[test]
fn prospective_narration_paraphrases_cannot_bypass_completion_guard() {
    // The guard deliberately does not classify prose with a phrase list. Once
    // tools have run, any text-only stop needs the structured completion marker,
    // so adjacent phrasings all take the same safe path.
    for _prospective_text in [
        "Now I will add the tests.",
        "Next, let me update the integration coverage.",
        "I should verify the remaining cases before finishing.",
    ] {
        let mut guard = ActionCompletionGuard::default();
        assert_eq!(
...
#[test]
fn completion_guard_is_bounded_and_resets_after_real_tool_progress() {
    let mut guard = ActionCompletionGuard::default();
    guard.observe(true, true, true, false);
    assert_eq!(
        guard.observe(true, false, false, false),
    ...
    for _ in 1..=MAX_COMPLETION_GUARD_REFUSALS {
        let decision = guard.observe(true, false, false, false);
        if guard.refusals < MAX_COMPLETION_GUARD_REFUSALS {
            assert_eq!(decision, CompletionGuardDecision::RequestExplicitDecision);
        } else {
            assert_eq!(decision, CompletionGuardDecision::Refused);
        }
    }
...
#[test]
fn informational_and_non_chat_turns_complete_without_guard_round_trip() {
    let mut informational = ActionCompletionGuard::default();
    assert_eq!(
        informational.observe(true, false, false, false),
        CompletionGuardDecision::ContinueNormally
    );

    let mut responses_api = ActionCompletionGuard::default();
    responses_api.observe(false, true, true, false);
    assert_eq!(
        responses_api.observe(false, false, false, false),
        CompletionGuardDecision::ContinueNormally
    );
}

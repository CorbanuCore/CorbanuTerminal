...
#[tokio::test]
async fn pre_tool_use_payload_accepts_function_wrapped_patch_input() {
    let patch = sample_patch();
...
#[test]
fn function_wrapped_patch_supports_object_and_string_arguments() {
    let patch = sample_patch();
    let payloads = [
    ...
    assert_eq!(
        payloads.map(|payload| apply_patch_payload_command(&payload)),
        [Some(patch.to_string()), Some(patch.to_string())]
    );
}

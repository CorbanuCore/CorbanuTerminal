...
impl ToolRegistry {
    ...
    pub(crate) async fn dispatch_any_with_terminal_outcome(
        ...
        if !tool.matches_kind(&invocation.payload) {
            ...
            // Provider adapters can encode a tool call using a payload shape that does not
            // match the advertised tool specification. This is recoverable model output, not
            // a host invariant: return a failed tool result so the model can correct the call
            // instead of terminating (and, in debug builds, panicking) the entire turn.
            let err = FunctionCallError::RespondToModel(message);
            dispatch_trace.record_failed(&err);
            return Err(err);
        }

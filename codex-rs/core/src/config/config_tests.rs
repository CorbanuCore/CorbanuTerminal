...
#[tokio::test]
async fn load_config_anthropic_provider_accepts_fable_5() -> std::io::Result<()> {
    let cfg = toml::from_str::<ConfigToml>(
        r#"
model_provider = "anthropic"
...
"#,
    ...
    assert_eq!(config.model_provider_id, ANTHROPIC_PROVIDER_ID);
    assert_eq!(config.model.as_deref(), Some(CLAUDE_FABLE_5_MODEL));
    assert_eq!(config.model_provider.wire_api, WireApi::Anthropic);
    assert_eq!(config.forced_login_method, Some(ForcedLoginMethod::Api));
...
#[tokio::test]
async fn load_config_claude_plan_provider_accepts_fable_5_plan() -> std::io::Result<()> {
    let cfg = toml::from_str::<ConfigToml>(
        r#"
model_provider = "claude-plan"

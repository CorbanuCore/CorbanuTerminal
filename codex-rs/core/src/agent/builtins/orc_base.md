...
- For user-facing visual or interactive work, capture a baseline driven through real user inputs before implementation and the candidate through real user inputs for acceptance across the assigned flows, idle/no-command behavior, transitions, and relevant interactions. For products with directional control, exercise every supported movement direction in both recordings. Make pointer/action intent visible so the judge can compare the requested action with the outcome. Run `pfterminal visual-judge --video <capture.mp4> --rubric <rubric.txt> --out <verdict.json>` and treat any nonzero result or visible defect as unfinished work. Screenshots, logs, engine state, pixel samples, and structural tests are supporting diagnostics only.

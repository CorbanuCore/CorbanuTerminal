...
enum Request { LoadJournal, LoadPolicy, Compare { expected: Option<Checkpoint>, next: Checkpoint } }

#[derive(Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
enum Reply { Loaded(Option<Checkpoint>), Stored, Rejected(RootError) }

#[derive(Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
struct Packet { sequence: u64, payload: Vec<u8>, tag: Vec<u8> }
...
fn peer(stream: &UnixStream) -> Result<libc::ucred, RootError> {
    ...
    if unsafe { libc::getsockopt(stream.as_raw_fd(), libc::SOL_SOCKET, libc::SO_PEERCRED, credentials.as_mut_ptr().cast(), &mut size) } != 0
        || size as usize != size_of::<libc::ucred>() { return Err(RootError::Unavailable); }
...
fn configure(stream: &UnixStream) -> Result<(), RootError> {
    stream.set_read_timeout(Some(Duration::from_secs(10))).map_err(|_| RootError::Unavailable)?;
    stream.set_write_timeout(Some(Duration::from_secs(10))).map_err(|_| RootError::Unavailable)
}

fn read<T: DeserializeOwned>(stream: &mut UnixStream) -> Result<T, RootError> {
    let mut prefix = [0; 4];
    stream.read_exact(&mut prefix).map_err(|_| RootError::Unavailable)?;
    let length = u32::from_be_bytes(prefix) as usize;
    if length > MAX_BYTES { return Err(RootError::Invalid); }
    let mut bytes = Zeroizing::new(vec![0; length]);
    stream.read_exact(&mut bytes).map_err(|_| RootError::Unavailable)?;
...
struct Channel { stream: UnixStream, key: Zeroizing<[u8; 32]>, sequence: u64 }
...
impl Channel {
    fn send<T: Serialize>(&mut self, value: &T, direction: &[u8]) -> Result<(), RootError> {
        ...
        mac.update(&payload);
        write(&mut self.stream, &Packet { sequence: self.sequence, payload, tag: mac.finalize().into_bytes().to_vec() })
    }

    fn receive<T: DeserializeOwned>(&mut self, direction: &[u8]) -> Result<T, RootError> {
        let packet: Packet = read(&mut self.stream)?;
        if packet.sequence != self.sequence { return Err(RootError::Invalid); }
        let mut mac = Hmac::<Sha256>::new_from_slice(self.key.as_ref()).map_err(|_| RootError::Invalid)?;
        mac.update(direction);
        mac.update(&packet.sequence.to_be_bytes());
        mac.update(&packet.payload);
        mac.verify_slice(&packet.tag).map_err(|_| RootError::Invalid)?;
...
fn alive(pidfd: &File) -> Result<(), RootError> {
    ...
    if unsafe { libc::poll(&mut poll, 1, 0) } == 0 { Ok(()) } else { Err(RootError::Unavailable) }
}
...
impl ControllerRoot {
    ...
    pub fn serve_child(&self, mut stream: UnixStream, child: &mut Child) -> Result<(), RootError> {
        if child.try_wait().map_err(|_| RootError::Unavailable)?.is_some() { return Err(RootError::Unavailable); }
        ...
        if raw < 0 { return Err(RootError::Unsupported); }
        // SAFETY: pidfd_open returned a newly owned descriptor.
        let pidfd = unsafe { File::from_raw_fd(raw as i32) };
        if child.try_wait().map_err(|_| RootError::Unavailable)?.is_some() || peer(&stream)?.pid != pid { return Err(RootError::Invalid); }
        alive(&pidfd)?;
        configure(&stream)?;
        ...
        let mut channel = Channel { stream, key, sequence: 1 };
        loop {
            let request: Request = channel.receive(b"corbanu-anchor-request/v1")?;
            alive(&pidfd)?;
            let reply = match request {
                Request::LoadJournal => { self.require_journal()?; Reply::Loaded(self.load()?) },
                Request::LoadPolicy => { self.require_policy()?; Reply::Loaded(self.load()?) },
                Request::Compare { expected, next } => {
                    match self.compare(expected.as_ref(), &next) {
                        Ok(()) => Reply::Stored,
                        Err(error) => { let _ = channel.send(&Reply::Rejected(error), b"corbanu-anchor-reply/v1"); return Err(error); }
                    }
                }
            };
            alive(&pidfd)?;
            channel.send(&reply, b"corbanu-anchor-reply/v1").map_err(|_| RootError::Ambiguous)?;
...
pub struct NativeAnchorClient { channel: Mutex<Option<Channel>> }

impl std::fmt::Debug for NativeAnchorClient {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result { f.debug_struct("NativeAnchorClient").finish_non_exhaustive() }
}
...
impl NativeAnchorClient {
    ...
    pub fn connect_system() -> Result<Self, RootError> {
        ...
        if metadata.uid() != 0 || metadata.file_type().is_symlink() { return Err(RootError::Invalid); }
        let stream = UnixStream::connect(SOCKET).map_err(|_| RootError::Unavailable)?;
        if peer(&stream)?.uid != 0 { return Err(RootError::Invalid); }
    ...
    fn from_authenticated_stream(mut stream: UnixStream) -> Result<Self, RootError> {
        ...
        Ok(Self { channel: Mutex::new(Some(Channel { stream, key: Zeroizing::new(key), sequence: 1 })) })
    }
    ...
    fn exchange(&self, request: &Request) -> Result<Reply, RootError> {
        ...
        let result = channel.send(request, b"corbanu-anchor-request/v1")
        ...
        match result.map_err(|error| if matches!(request, Request::Compare { .. }) { RootError::Ambiguous } else { error })? {
            Reply::Rejected(error) => { *slot = None; Err(error) },
            reply => Ok(reply),
        }
    }
}
...
impl crate::PolicyRootStore for NativeAnchorClient {
    ...
    fn compare_policy(&self, expected: Option<&PolicyCheckpoint>, next: &PolicyCheckpoint) -> Result<(), RootError> {
        match self.exchange(&Request::Compare { expected: expected.cloned().map(Checkpoint::Policy), next: Checkpoint::Policy(next.clone()) })? {
            Reply::Stored => Ok(()), _ => Err(RootError::Invalid),
        }
    }
}
...
impl IntegrityRootStore for NativeAnchorClient {
    ...
    fn compare_and_store(&self, expected: Option<&IntegrityCheckpoint>, next: &IntegrityCheckpoint) -> Result<(), IntegrityRootError> {
        match self.exchange(&Request::Compare { expected: expected.cloned().map(Checkpoint::Journal), next: Checkpoint::Journal(next.clone()) })? {
            Reply::Stored => Ok(()), _ => Err(IntegrityRootError::Invalid),
        }
    }
}

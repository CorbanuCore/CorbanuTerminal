...
fn owner() -> JournalOwner {
    JournalOwner::new(PolicyPrincipal::new(PrincipalKind::Service, "test-controller").unwrap(), 1, BoundedText::new("test-key").unwrap()).unwrap()
}

fn checkpoint(sequence: u64) -> IntegrityCheckpoint {
    let owner = owner();
    IntegrityCheckpoint { schema_version: 1, sequence, record_sha256: "a".repeat(64), producer: owner.producer().clone(), owner_generation: 1, integrity_key_id: owner.integrity_key_id().clone(), policy_generation: 1, run_generation: 1 }
}
...
fn fixture() -> (tempfile::TempDir, ControllerRoot) {
    ...
    let root = ControllerRoot::enroll(&temp.path().join("registry"), &temp.path().join("storage"), Enrollment::journal(&owner()).0).unwrap();
    (temp, root)
}
...
fn pf20_s03_enrollment_cas_restart_and_wrong_namespace() {
    ...
    assert_eq!(root.compare_and_store(None, &checkpoint(2)), Err(IntegrityRootError::Conflict));
    let mut wrong = first.clone(); wrong.record_sha256 = "b".repeat(64);
    assert_eq!(root.compare_and_store(Some(&wrong), &checkpoint(2)), Err(IntegrityRootError::Conflict));
    root.compare_and_store(Some(&first), &checkpoint(2)).unwrap();
    assert_eq!(ControllerRoot::open(&temp.path().join("registry"), &temp.path().join("storage")).unwrap_err(), RootError::Conflict);
    drop(root);
    let restarted = ControllerRoot::open(&temp.path().join("registry"), &temp.path().join("storage")).unwrap();
    assert_eq!(IntegrityRootStore::load(&restarted), Ok(Some(checkpoint(2))));
    assert_eq!(ControllerRoot::enroll(&temp.path().join("registry"), &temp.path().join("storage"), Enrollment::journal(&owner()).0).unwrap_err(), RootError::Unavailable);
}
...
fn pf20_s03_foreign_binding_generation_and_sequence_never_commit() {
    ...
    assert_eq!(root.compare_and_store(Some(&first), &wrong), Err(IntegrityRootError::Invalid));
    wrong = checkpoint(2); wrong.integrity_key_id = BoundedText::new("foreign").unwrap();
    assert_eq!(root.compare_and_store(Some(&first), &wrong), Err(IntegrityRootError::Invalid));
    wrong = checkpoint(2); wrong.run_generation = 0;
    assert_eq!(root.compare_and_store(Some(&first), &wrong), Err(IntegrityRootError::Invalid));
    assert_eq!(root.compare_and_store(Some(&first), &checkpoint(3)), Err(IntegrityRootError::Invalid));
...
fn pf20_s03_lost_corrupt_key_registry_head_and_partial_enrollment_deny() {
    for target in ["registry/enrollment", "registry/complete", "storage/key", "storage/head", "storage/lock"] {
        let (temp, root) = fixture();
        drop(root);
        fs::remove_file(temp.path().join(target)).unwrap();
        assert!(ControllerRoot::open(&temp.path().join("registry"), &temp.path().join("storage")).is_err(), "{target}");
    }
    for target in ["registry/complete", "storage/key", "storage/head"] {
        ...
        drop(root);
        assert!(ControllerRoot::open(&temp.path().join("registry"), &temp.path().join("storage")).is_err());
    }
    let (temp, root) = fixture(); drop(root);
    fs::remove_file(temp.path().join("registry/complete")).unwrap();
    assert!(ControllerRoot::enroll(&temp.path().join("registry"), &temp.path().join("storage"), Enrollment::journal(&owner()).0).is_err());
}
...
fn pf20_s03_torn_pending_and_symlink_replacements_latch_unavailable() {
    ...
    assert_eq!(IntegrityRootStore::load(&root), Err(IntegrityRootError::Timeout));
    ...
    drop(root);
    let root = ControllerRoot::open(&temp.path().join("registry"), &temp.path().join("storage")).unwrap();
    fs::rename(temp.path().join("storage/head"), temp.path().join("storage/head-copy")).unwrap();
...
fn pf20_s03_policy_anchor_retains_exact_existing_payload() {
    let (temp, old) = fixture(); drop(old);
    let policy_registry = temp.path().join("policy-registry");
    let policy_storage = temp.path().join("policy-storage");
    for dir in [&policy_registry, &policy_storage] { fs::create_dir(dir).unwrap(); fs::set_permissions(dir, fs::Permissions::from_mode(0o700)).unwrap(); }
    let owner = AuthoritativeStateOwner::new("1".repeat(64), "controller", 1).unwrap();
    let root = ControllerRoot::enroll(&policy_registry, &policy_storage, Enrollment::policy(owner.clone()).unwrap().0).unwrap();
    assert_eq!(root.load_policy(), Ok(None));
    assert_eq!(IntegrityRootStore::load(&root), Err(IntegrityRootError::Invalid));
    let value = PolicyCheckpoint { schema_version: 1, revision: 1, owner, state_sha256: "a".repeat(64), commit_sha256: "b".repeat(64) };
    root.compare_policy(None, &value).unwrap();
    assert_eq!(root.load_policy(), Ok(Some(value.clone())));
    drop(root);
    assert_eq!(ControllerRoot::open(&policy_registry, &policy_storage).unwrap().load_policy(), Ok(Some(value)));
}

#[test]
fn pf20_s03_real_second_process_cannot_acquire_controller_lock() {
    let (temp, _root) = fixture();
    let status = Command::new(std::env::current_exe().unwrap()).args(["--exact", "store::tests::lock_child", "--ignored"])
        .env("CORBANU_ANCHOR_LOCK_FIXTURE", temp.path()).status().unwrap();
...
fn lock_child() {
    ...
    assert_eq!(ControllerRoot::open(&path.join("registry"), &path.join("storage")).unwrap_err(), RootError::Conflict);
}

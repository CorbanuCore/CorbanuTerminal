# App-server development guide

This is the shared authority for `codex-rs/app-server/` and
`codex-rs/app-server-protocol/`. Their nested `AGENTS.md` files require this
guide.
...
- Exercise the app-server's public JSON-RPC API.
- Use the same server-mocking patterns as core integration tests.
- Use `TestAppServer::builder().build()` and
  `TestAppServer::send_thread_start_request_with_auto_env()` by default so tests
  work with foreign app/exec operating systems. See the

...
pub(crate) fn huihui_deepseek_v4_flash_recipe() -> GpuRecipe {
    ...
    let launch = format!(
        concat!(
            ...
            "pft_phase() {{ printf '%s\\n' \"$1\" > /tmp/pfterminal-provision-phase.new; mv /tmp/pfterminal-provision-phase.new /tmp/pfterminal-provision-phase; }}; ",
            "pft_phase hardware_check; ",
            ...
            "printf 'PFTERMINAL_RUNTIME_GATE=server-ok\\n'; pft_phase endpoint_probing; /opt/caddy run --config /tmp/Caddyfile --adapter caddyfile & proxy_pid=$!; ",
    ...
    GpuRecipe {
        ...
        revision: "huihui-deepseek-v4-flash-q4k-ds4-tp2-r4".to_string(),
...
pub(crate) fn huihui_glm_5_2_recipe() -> GpuRecipe {
    ...
    let launch = format!(
        concat!(
            ...
            "printf 'PFTERMINAL_RUNTIME_GATE=cuda-ok\\n'; mkdir -p /models/glm52; pft_phase model_download; ",
    ...
    GpuRecipe {
        ...
        revision: "huihui-glm-5.2-iq1m-llamacpp-2xh200-r5".to_string(),
        model_id: model_id.to_string(),

...
fn pf_30_s01_round_trip_retains_untrusted_lineage() {
    ...
    assert_eq!(serde_json::from_value::<SourceEnvelope>(wire).unwrap(), expected);
...
fn pf_30_s01_metadata_controls_and_unknown_kind_are_rejected() {
    for metadata in ["system\nrole=human", "safe\u{202e}txt", "safe\u{200b}", "<system>"] {
        let mut wire = serde_json::to_value(envelope()).unwrap();
...
fn pf_30_s01_broken_transformation_and_excess_lineage_fail() {
    ...
    assert_eq!(
        SourceEnvelope::new(Uuid::from_u128(1), source.clone(), [1; 32], vec![SourceTransformation {
            id: "normalize-v1".into(), input_digest: [2; 32], output_digest: [3; 32],
        }], vec![]),
        Err(ProvenanceError::InvalidTransformation),
    );
    assert_eq!(
        SourceEnvelope::new(Uuid::from_u128(1), source, [1; 32], vec![], vec![Uuid::from_u128(2); MAX_SOURCE_LINEAGE]),

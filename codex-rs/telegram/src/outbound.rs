...
mod tests {
    ...
    fn mutating_calls_are_never_retried() {
        assert_eq!(
            retry_decision(CallSafety::Mutating, &rate_limit_error(/*secs*/ 1), /*attempt*/ 0),
            None
        );
        assert_eq!(
    ...
    fn idempotent_429_honors_retry_after_floor() {
        let delay = retry_decision(CallSafety::Idempotent, &rate_limit_error(/*secs*/ 5), /*attempt*/ 0)
    ...
    fn idempotent_transport_uses_bounded_backoff() {
        let first = retry_decision(CallSafety::Idempotent, &transport_error(), /*attempt*/ 0).unwrap();
        let second = retry_decision(CallSafety::Idempotent, &transport_error(), /*attempt*/ 1).unwrap();
        assert!(second > first);

...
fn group_chat_requires_an_explicit_authorized_actor() {
    ...
    assert!(allowlist.reject_if_unauthorized_actor(ChatId(-100_123), Some(UserId(7)), /*is_private*/ false));
    assert!(!allowlist.reject_if_unauthorized_actor(ChatId(-100_123), Some(UserId(8)), /*is_private*/ false));
    assert!(!allowlist.reject_if_unauthorized_actor(ChatId(-100_123), /*user_id*/ None, /*is_private*/ false));
}
...
fn private_chat_retains_chat_allowlist_behavior() {
    ...
    assert!(allowlist.reject_if_unauthorized_actor(ChatId(42), Some(UserId(42)), /*is_private*/ true));
}

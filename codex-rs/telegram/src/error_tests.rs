...
fn backoff_doubles_and_caps() {
    let mut backoff = PollingBackoff::new(Duration::from_secs(1), Duration::from_secs(4), /*max_consecutive_failures*/ 10);
...
fn backoff_cap_uses_failure_cap_error() {
    let mut backoff = PollingBackoff::new(Duration::from_secs(1), Duration::from_secs(4), /*max_consecutive_failures*/ 2);

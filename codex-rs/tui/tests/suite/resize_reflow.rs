...
async fn tmux_smoke_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let split_pane = session.split_vertical(
        codex_pane,
        /*rows*/ 12,
        CommandSpec::new("sleep").arg("30"),
    )?;
    let split_capture = codex_pane.wait_stable_contains(draft, Duration::from_secs(/*secs*/ 15))?;
    let split_row = last_composer_row(&split_capture).context("composer row after split")?;
    anyhow::ensure!(
        split_row <= baseline_row + 1,
        "composer row snapped downward after split: baseline={baseline_row}, after={split_row}\n\
         baseline:\n{baseline_capture}\n\
         after:\n{split_capture}"
    );

    split_pane.close()?;
    let final_capture = codex_pane.wait_stable_until(
        "composer and history rows to return to their pre-split positions",
...
fn codex_binary(repo_root: &Path) -> Result<PathBuf> {
    ...
    anyhow::bail!(
        "Codex TUI binary is unavailable; run `cargo build -p codex-cli --bin codex` or \
         `cargo build -p codex-tui --bin codex-tui` first"
    )
}

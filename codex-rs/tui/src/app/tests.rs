    ...
    runtime.block_on(async {
        ...
        assert_eq!(
            ...
            Some(&AgentPickerThreadEntry {
                ...
                catalogue_summary: Some(
                    "capability frontier · billing auth-dependent (plan 1× burn or API $5/M input / $30/M output) · vision"
                        .to_string(),
                ),
                is_running: false,
                is_closed: false,
...
fn selected_and_resumed_threads_use_server_capability_for_v1_and_v2_children() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        app.select_agent_thread(&mut tui, &mut app_server, child_thread_ids[0])
            .await?;
        let selection_messages = std::iter::from_fn(|| app_event_rx.try_recv().ok())
            ...
            .collect::<Vec<_>>();
        assert!(selection_messages.iter().any(|message| {
            message.contains("Viewing spawned agent `/root/child-0`")
                && message.contains("separate billable activity")
        }));
        app.chat_widget
...
async fn inactive_thread_started_notification_initializes_replay_session() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some(&AgentPickerThreadEntry {
            ...
            catalogue_summary: Some(
                "catalogue evidence unavailable for this exact route".to_string(),
            ),
            is_running: false,
            is_closed: false,
        })
...
fn acknowledged_thread_settings(
    model: &str,
    provider: &str,
    effort: Option<ReasoningEffortConfig>,
) -> ThreadSettings {
    ThreadSettings {
        cwd: test_absolute_path("/tmp/thread-settings-ack"),
        approval_policy: AskForApproval::OnRequest,
...
#[tokio::test]
async fn model_selection_persists_only_after_exact_runtime_acknowledgement() {
    let (mut app, mut app_event_rx, _op_rx) = make_test_app_with_channels().await;
    let thread_id = ThreadId::new();
    app.pending_model_selection = Some(PendingModelSelection::Catalog {
        thread_id,
        model: "gpt-5.6-sol".to_string(),
        effort: Some(ReasoningEffortConfig::XHigh),
    });

    app.commit_pending_model_selection_after_ack(
        thread_id,
        &acknowledged_thread_settings(
            "gpt-5.6-terra",
            "openai",
    ...
    assert_matches!(
        app_event_rx.try_recv().expect("matching acknowledgement should persist"),
        AppEvent::PersistModelSelection { model, effort }
            if model == "gpt-5.6-sol" && effort == Some(ReasoningEffortConfig::XHigh)
    );
    assert!(app.pending_model_selection.is_none());
}

#[tokio::test]
async fn provider_selection_requires_exact_provider_and_model_acknowledgement() {
    let (mut app, mut app_event_rx, _op_rx) = make_test_app_with_channels().await;
    let thread_id = ThreadId::new();
    app.pending_model_selection = Some(PendingModelSelection::Provider {
        thread_id,
    ...
    app.commit_pending_model_selection_after_ack(
        thread_id,
        &acknowledged_thread_settings("claude-opus-5-plan", "openai", /*effort*/ None),
    );
    assert!(app_event_rx.try_recv().is_err());

    app.commit_pending_model_selection_after_ack(
        thread_id,
        &acknowledged_thread_settings("claude-opus-5-plan", "claude-plan", /*effort*/ None),
    );
    assert_matches!(
        app_event_rx.try_recv().expect("exact route acknowledgement should persist"),
        AppEvent::PersistProviderModelSelection { model, provider }
            if model == "claude-opus-5-plan" && provider == "claude-plan"
    );
    assert!(app.pending_model_selection.is_none());
}
...
async fn thread_setting_update_params_sync_model_and_default_reasoning() {
    ...
    assert_eq!(
        app.chat_widget.current_model(),
        original_model,
        "request construction must not change the visible model before acknowledgement"
    );
    ...
    let atomic_params = app
        .active_thread_model_and_reasoning_setting_update_params(
            "gpt-5.4".to_string(),
            Some(ReasoningEffortConfig::High),
        )
        .expect("active thread should produce atomic model settings");
    assert_eq!(atomic_params.model, Some("gpt-5.4".to_string()));
    assert_eq!(atomic_params.effort, Some(ReasoningEffortConfig::High));
    let atomic_mode = atomic_params
        .collaboration_mode
        .expect("atomic update should carry one coherent mode");
    assert_eq!(atomic_mode.settings.model, "gpt-5.4");
    assert_eq!(
        atomic_mode.settings.reasoning_effort,
        Some(ReasoningEffortConfig::High)
    );

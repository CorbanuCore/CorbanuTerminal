...
pub(crate) async fn resolve_claude_oauth_access_token() -> Result<String> {
    ...
    resolve_stored_claude_oauth_access_token(&config_dir, /*claude_executable*/ None, current_time_ms(), force_refresh)
        .await
}
...
mod tests {
    ...
    async fn expiring_access_token_is_refreshed_by_claude_cli() {
        ...
        let claude = fake_refreshing_claude(temp_dir.path(), now_ms + 600_000, /*sleep_seconds*/ 0, /*exit_code*/ 0);

        let token =
            resolve_stored_claude_oauth_access_token(temp_dir.path(), Some(&claude), now_ms, /*force_refresh*/ false)
                .await
    ...
    async fn concurrent_refreshes_share_one_claude_exchange() {
        ...
        let claude = fake_refreshing_claude(temp_dir.path(), now_ms + 600_000, /*sleep_seconds*/ 1, /*exit_code*/ 0);
        ...
        let (first, second) = tokio::join!(
            resolve_stored_claude_oauth_access_token(&config_a, Some(&claude_a), now_ms, /*force_refresh*/ false,),
            resolve_stored_claude_oauth_access_token(&config_b, Some(&claude_b), now_ms, /*force_refresh*/ false),
        );
    ...
    async fn forced_refresh_replaces_an_unexpired_rejected_token() {
        ...
        let claude = fake_refreshing_claude(temp_dir.path(), now_ms + 900_000, /*sleep_seconds*/ 0, /*exit_code*/ 0);

        let token =
            resolve_stored_claude_oauth_access_token(temp_dir.path(), Some(&claude), now_ms, /*force_refresh*/ true)
                .await
    ...
    async fn rotated_credentials_win_over_nonzero_claude_exit() {
        ...
        let claude = fake_refreshing_claude(temp_dir.path(), now_ms + 600_000, /*sleep_seconds*/ 0, /*exit_code*/ 1);

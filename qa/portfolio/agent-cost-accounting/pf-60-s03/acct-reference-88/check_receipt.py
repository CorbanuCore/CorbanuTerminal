...
receipt = json.loads((here / "advanced/receipt.json").read_text())
...
record = dict(assertions=["output-match", "stable-tip", "clean-checkout",
                          "command-exit", "empty-stderr"],
              positive_controls=1, negative_controls=len(mutations), cases=results)
(here / "receipt-controls.json").write_text(json.dumps(record, indent=2) + "\n")

...
for relative in (
    ...
    "acct-selfcheck-84/check_current_tip.py", "acct-decay-87/refresh_inventory.py",
    "acct-fitness-76/finalize.py",
    "acct-acceptance-75/acceptance.md", "acct-acceptance-75/scope.json",
    "acct-inventory-79/inventory-correction.json",
):
...
current = json.loads((here / "current-tip/receipt.json").read_text())
reference = subprocess.check_output(["git", "show",
    current["reference_commit"] + ":" + current["reference"]], cwd=repo)
assert hashlib.sha256(reference).hexdigest() == current["reference_sha256"]
assert (here / "current-tip/acceptance.stdout.txt").read_bytes() == reference
assert current["stdout_matches_prior_replay"]
assert current["commit"] == current["source_tip_after"] == subprocess.check_output(
    ["git", "rev-parse", current["integration_ref"]], cwd=repo, text=True).strip()
for row in current["commands"]:
    for stream in ("stdout", "stderr"):
        raw = (here / "current-tip" / (row["name"] + "." + stream + ".txt")).read_bytes()
        assert hashlib.sha256(raw).hexdigest() == row[stream + "_sha256"]
    assert row["exit"] == row["expected_exit"] and row["stderr_empty"]
match_checks = json.loads((here / "match-flag-checks.json").read_text())
assert len(match_checks) == 3 and all(row["passed"] for row in match_checks)

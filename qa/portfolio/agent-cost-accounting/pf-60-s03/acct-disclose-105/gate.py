...
if name != "prerequisites":
    if json.loads((out / "prerequisites.json").read_text())["exit"] != 0:
        raise RuntimeError("prerequisite build did not pass")
settings = dict(
...
record = dict(lane=name, command=commands[name], cwd=str(repo / "codex-rs"),
              ...
              isolation_banner="Test isolation: disposable profile; native keyring disabled (debug lane)" in text,
...
path.unlink()  # Lossless gzip and SHA-256 retained; avoid duplicate raw logs.

...
Receipt repair: historical acct-activation-33 log links were replaced with inline nextest summaries and compiler diagnostics read from the original logs. This receipt likewise inlines essential evidence; raw local `*.txt` attempts are explicitly ignored, not linked as committed evidence.

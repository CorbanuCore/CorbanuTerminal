...
try:
 ...
 assert requests[0].get('tools'), 'Tools must be present for the omission check'
 assert all('parallel_tool_calls' not in r for r in requests), 'Unsupported parallel control leaked into a request'
 stop();launch();send('/model');s=wait('DeepSeek V4.1 Flash (current)');(ROOT/'live-restart.txt').write_text(s.rstrip()+'\n');keys('Escape')
 result={'ok':True,'model':MODEL,'binary':str(BIN),'checks':['visible in Corbanu API','cancel preserves current model','selection persists Corbanu provider and model','High reasoning and streaming request use exact public model','preparation failure retries with a fresh ID before upstream dispatch','parallel control omitted with tools present','real tool execution and result round trip','response renders','restart retains selected model'],'gatewayRequests':len(requests),'statuses':statuses,'releasedAttemptRotated':request_ids[0]!=request_ids[1],'realToolResultRoundTrip':True,'productionWrites':0}

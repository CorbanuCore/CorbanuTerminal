/* Non-root synthetic x86_64 relay. Parent PID names rendezvous, never authority.
 * No libc, shell, configurable executable/destination, secrets or UID changes. */
...
void start(long *stack) {
    ...
    if (*stack != 6 || argv[2][0] != 'j') die(90);
    struct address address = {1, {0}};
    const char prefix[] = "corbanu-pf27-root-";
    int len = 1;
    ...
    for (;;) {
        ...
        if (ready == -4) continue;
        if (ready <= 0) die(93);
        /* Control first: acknowledge pause before any later root reply. */
        if (peers[2].revents) {
            char command;

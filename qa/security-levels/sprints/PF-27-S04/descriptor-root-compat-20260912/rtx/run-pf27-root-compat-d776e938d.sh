...
export PF27_PAIR_REPO=/home/travis/worktrees/security-broker-root-compat-20260912
...
proof=/home/travis/security-round5/evidence/pf27-root-compat-20260912
export PF27_EVIDENCE_DIR="$proof/final-tmux"
test -n "${TMUX:-}"
test "$(id -u)" = 1001
mkdir "$proof/final-started"
cd "$PF27_PAIR_REPO"
test "$(git rev-parse HEAD)" = d776e938d8a0c07cc9b50b1d6a818d9f6feb0c11
test "$(git rev-parse HEAD:codex-rs)" = b48e9c4f9cb89e814b8bdabd6069be86cc47badd
...
test "$(sha256sum "$PF27_ELF_INTERPRETER" | cut -d ' ' -f1)" = 37fb0d7c5fd7e09ca23e9a5b1901b77786b87dfe8a869e191452c571bda7081d
{
    ...
    stat -f -c '%T' "$TMPDIR"
} > "$proof/final-provenance.txt"
sha256sum codex-rs/Cargo.toml codex-rs/Cargo.lock MODULE.bazel MODULE.bazel.lock > "$proof/modules-before.txt"
result=0
bash qa/security-levels/sprints/PF-27-S04/descriptor-root-compat-20260912/qualify-rtx.sh > "$proof/final-suite.log" 2>&1 || result=$?
printf '%s\n' "$result" > "$proof/final-suite.exit"
tail -50 "$proof/final-suite.log"
...
test -z "$(git status --porcelain)"
test "$result" = 0
printf 'PF27_ROOT_COMPAT_D776E938D_COMPLETE\n'

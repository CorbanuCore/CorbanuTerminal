#!/usr/bin/env bash
set -euo pipefail
repo=/home/travis/worktrees/security-broker-root-composition-20260912
evidence=${PF27_EVIDENCE_DIR:-/home/travis/security-round5/evidence/pf27-root-composition-20260912/verified}
mkdir -p "$evidence"
...
export TMPDIR
TMPDIR=$(mktemp -d /home/travis/.pf27r.XXXXXX)
cd "$repo"
...
run fmt just fmt
# Full transitive Clippy currently fails on pre-existing codex-api telemetry
# lints. Preserve the original failure separately; these are changed-crate checks.

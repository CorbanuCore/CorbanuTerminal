...
repo = Path('/home/travis/worktrees/security-broker-sealed-20260912')
evidence = Path(os.environ.get('PF27_EVIDENCE_DIR', '/home/travis/security-round5/evidence/pf27-sealed-20260912/verified'))
...
with tempfile.TemporaryDirectory(prefix='.pf27c-', dir='/home/travis') as temp:
    ...
    try:
        ...
        assert '39 tests run: 39 passed' in lifecycle
        ...
        (evidence / 'tmux-result.json').write_text(json.dumps({
            'passed': True, 'tests': 39, 'production_exit': 78, 'preparation_nonroot_exit': 78,
            'candidate': str(candidate), 'native_isolation_proven': False,
            'privileged_installation': False, 'user_facing_tui': False,
        }, indent=2) + '\n')
    finally:
        tmux('kill-server', check=False)
print('PF27 sealed image TMUX checks passed')

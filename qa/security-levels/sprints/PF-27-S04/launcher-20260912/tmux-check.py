...
repo = Path('/home/travis/worktrees/security-broker-launch-20260912')
evidence = Path(os.environ.get('PF27_EVIDENCE_DIR', '/home/travis/security-round5/evidence/pf27-launch-20260912/verified'))
...
with tempfile.TemporaryDirectory(prefix='.pf27c-', dir='/home/travis') as temp:
    ...
    try:
        ...
        command(f'{shlex.quote(str(probe))} --prepare-synthetic-child journal 101 201 204; printf "\\nprepare-exit=%s\\n" "$?"')
        preparation_denial = wait_for('prepare-exit=78')
        assert 'post-exec inspection failed' in preparation_denial
        (evidence / 'tmux-preparation-denial.txt').write_text(preparation_denial)
        command(
        ...
        assert '29 tests run: 29 passed' in lifecycle
        ...
        (evidence / 'tmux-result.json').write_text(json.dumps({
            'passed': True, 'tests': 29, 'production_exit': 78, 'preparation_nonroot_exit': 78,
            'candidate': str(candidate), 'native_isolation_proven': False,
            'privileged_installation': False, 'user_facing_tui': False,
        }, indent=2) + '\n')
    finally:
        tmux('kill-server', check=False)
print('PF27 launch recipe / preparation TMUX checks passed')

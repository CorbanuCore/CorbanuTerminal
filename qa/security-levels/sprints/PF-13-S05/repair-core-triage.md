...
| Prompt-cache context transition | `overrides_turn_context_but_keeps_cached_prefix_and_key_constant` | One expected request was absent. This failure was not in the prior failed set; isolated rerun/transition tracing remains necessary. No regression attribution inferred from one full run. |

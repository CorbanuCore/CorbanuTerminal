...
def dispatch_loop(config, side, base):
    ...
    while time.monotonic() < end and not (base / "stop").exists() and not (base / ("stop-" + side)).exists():
        try:
...
def main():
    base = Path(tempfile.mkdtemp(prefix="oc85-", dir="/private/tmp"))
    ...
    authority = dict(decision_id="owner-cutover-85-disposable", revision=1, authority="synthetic rehearsal only",
    ...
    try:
        ...
        # Stop only the manual lane. Owner recurrence stays alive while a new,
        # default-hand external claim crosses its deadline.
        f.write_file(base / "stop-hand", b"stop manual passes")
        children[1].wait(timeout=15)
        assert children[1].returncode == 0 and children[0].poll() is None
        allocation = seed()[2]["bootstrap"]
        allocation["resources"] = ["overdue-hand"]
        c.put_allocation("overdue-hand", allocation, False, c.snapshot()["revision"], {"fixture": True})
        c.event({"id": "prepare-overdue-hand"})
        packet = c.begin_manager()
        action = dict(id="overdue-hand", kind="repair", workstream="delivery", sprint="PF80",
                      rationale="external default-hand overdue probe",
                      inputs={"allocation": "overdue-hand", **allocation["inputs"]},
                      timeout_seconds=60, expected_revision=packet["state_revision"])
        c.accept_decision(packet["manager_run"],
                          dict(state_revision=packet["state_revision"], actions=[action]), {"fixture": True})
        c.claim("overdue-hand")
        # A short synthetic deadline avoids waiting the production lease; no
        # ownership key is added, so this proves the new-action default too.
        with c.mutation("fixture-short-deadline", {}) as (_, snapshot):
            snapshot["actions"]["overdue-hand"]["deadline"] = time.time() + 2
        before = owner.activation_status(config_path)
        row = next(a for a in before["coordinator"]["in_flight"] if a["id"] == "overdue-hand")
        assert not row["overdue"] and not row["watchdog_covered"]
        assert before["coordinator"]["watchdog_coverage"]["excluded_actions"] == ["hand-work", "overdue-hand"]
        emit("before_hand_deadline", status=before)
        ...
        def pass_lane(side):
            ...
            while True:
                try:
                    return owner.Kernel(config_path, dispatcher=side).tick()
                except BlockingIOError:
                    assert time.monotonic() < end
                    time.sleep(0.05)

        overdue_status = owner.activation_status(config_path)
        row = next(a for a in overdue_status["coordinator"]["in_flight"] if a["id"] == "overdue-hand")
        assert row["overdue"] and not row["watchdog_will_report"]
        assert row["watchdog_status"] == "dispatching"
        before_action = c.snapshot()["actions"]["overdue-hand"]
        tick = pass_lane("owner")
        assert tick["state"] == "ACTIVE" and "overdue-hand" not in tick["actions"]
        assert c.snapshot()["actions"]["overdue-hand"] == before_action
        with c.connection() as db:
            pending = [json.loads(row[0]) for row in db.execute("SELECT body FROM events")]
        assert not any(e.get("action") == "overdue-hand" for e in pending)
        emit("owner_overdue_hand_unchanged", status=overdue_status, tick=tick, action=before_action)
        hand_tick = pass_lane("hand")
        assert hand_tick["actions"]["overdue-hand"] == "external_hand_claim"
        after_action = c.snapshot()["actions"]["overdue-hand"]
        assert after_action["status"] == "dispatch_uncertain" and after_action["stall_reported"]
        assert after_action["claim"] == before_action["claim"]
        with c.connection() as db:
            events = [json.loads(row[0]) for row in db.execute("SELECT body FROM events")]
        stalls = [e for e in events if e.get("action") == "overdue-hand"]
        assert len(stalls) == 1 and stalls[0]["id"].startswith("stall:")
        assert len(list(runs.glob("*/worker.json"))) == 2
        emit("manual_hand_watchdog_reported", tick=hand_tick, action=after_action, events=stalls)
        with sqlite3.connect(state / "owner.sqlite3") as db:
            assert db.execute("SELECT COUNT(*) FROM operations").fetchone()[0] == 22

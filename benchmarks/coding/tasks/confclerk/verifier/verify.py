...
BUG_PROBES = [
    'BugProbeTests.test_probe_bug1_false_env_disables_optional_partial',
'BugProbeTests.test_probe_bug2_replace_marker_replaces_nested_map',
'BugProbeTests.test_probe_bug3_delete_marker_removes_stale_secret'
]
...
def main() -> int:
    ...
    if not (workspace / "src" / "confclerk").exists():
        summary = {"ok": False, "error": f"not a confclerk workspace: {workspace}", "tests_passed": 0, "tests_total": 23, "bug_probes_passed": 0, "bug_probes_total": len(BUG_PROBES), "test_modified": True}
        print("CONFCLERK_VERIFIER_SUMMARY", json.dumps(summary, sort_keys=True))
        return 2

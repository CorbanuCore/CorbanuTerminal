...
BUG_PROBES = [
    "BugProbeTests.test_probe_bug1_env_false_override_disables_task",
    "BugProbeTests.test_probe_bug2_topological_order_for_independent_ready_nodes_is_sorted",
    "BugProbeTests.test_probe_bug3_retry_max_attempts_is_total_attempts_not_retries",
    "BugProbeTests.test_probe_bug4_resume_hydrates_checkpoint_outputs_for_downstream_tasks",
    "BugProbeTests.test_probe_bug5_metrics_count_skipped_tasks_and_retry_failures",
]
...
def main() -> int:
    ...
    if not (workspace / "src/pipeflow").exists():
        summary = {"ok": False, "error": f"not a pipeflow workspace: {workspace}", "tests_passed": 0, "tests_total": 31, "bug_probes_passed": 0, "bug_probes_total": 5, "test_modified": True}
        print("LIMITPRESS_VERIFIER_SUMMARY", json.dumps(summary, sort_keys=True))
        return 2

---
title: "Text-improvement harness qualification"
status: draft
...
# Text-improvement harness qualification
...
- Gate: Identify the real harness, sample permissions, target voice and exact approved model/provider before any remote evaluation.
...
A newer model may improve the text harness, but polished wording can quietly distort facts, intent and voice.

## Product intent and ideal flow

Evaluate the existing harness with paired, blinded examples and specify a reversible upgrade only if it improves accepted output.
...
- Feature: **PF-73**; outcome: Evaluate the existing harness with paired, blinded examples and specify a reversible upgrade only if it improves accepted output.
- Source: transcript discussion at 01:38:00–01:39:36; see portfolio for attribution caveats.
...
- In: Harness inventory and editorial rubric; Blinded exact-model comparison; Upgrade or retain decision.
- Out: Rebuilding a missing harness from guesswork, publishing text, sending private drafts to unapproved providers or automatic model switching.
...
| --- | --- | --- | --- | --- |
| Editorial workflow owner (assignment pending) | UNALLOCATED | UNALLOCATED | UNALLOCATED | PF-73; one sprint at a time |
...
All records belong to the single feature **PF-73**. Dependencies are hard prerequisites, not suggestions. All results are pending.

| Sprint | Record / outcome | Depends on | Planned output | Evidence |
| --- | --- | --- | --- | --- |
| PF-73-S01 | [Harness inventory and editorial rubric](../../sprints/current/portfolio-text-improvement/pf-73-s01-harness-inventory-and-editorial-rubric.md) | none | docs/research/text-improvement/rubric.md | pending |
| PF-73-S02 | [Blinded exact-model comparison](../../sprints/current/portfolio-text-improvement/pf-73-s02-blinded-exact-model-comparison.md) | PF-73-S01 | docs/research/text-improvement/comparison.md | pending |
| PF-73-S03 | [Upgrade or retain decision](../../sprints/current/portfolio-text-improvement/pf-73-s03-upgrade-or-retain-decision.md) | PF-73-S02 | docs/research/text-improvement/decision.md | pending |

## Acceptance flows

| Flow | Starting state / action | Expected result and pass criterion |
| --- | --- | --- |
| Success | Approved inputs; execute bounded sprint sequence | Editors prefer the candidate while factual claims, author intent and protected text remain intact. |
| Failure/cancel | Missing input, rejected gate or interrupted work | Fact mutation, invented citations, instruction injection and changed quotations are rejected even when style improves. |
| Recovery/resume | Reopen from a recorded checkpoint | Restore the original text and rerun from a versioned sample/model/prompt manifest. |

## Implementation sequence

1. **Harness inventory and editorial rubric:** Every sample has immutable original text and pass/fail fidelity criteria.
2. **Blinded exact-model comparison:** A preferred variant passes every critical fidelity gate; unavailable routes remain unavailable, not inferred.
3. **Upgrade or retain decision:** The decision cites examples and fidelity results, not model reputation.
...
- Identify the real harness, sample permissions, target voice and exact approved model/provider before any remote evaluation.

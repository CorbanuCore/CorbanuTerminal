...
| Credential-store liveness | regression | Codex primary integration agent | PF-57 | The locked-keyring flow requires a bounded shared keyring operation. Separately, the combined `codex-login` suite reproduced two 60-second timeouts; a live stack sample placed the CPU-bound wait in repeated unoptimized age/scrypt rewrites during one vault mutation. Preserve encrypted-vault fallback, work factor, and format while bounding keyring waits, coalescing each logical mutation, and optimizing only the development/test crypto implementation. |

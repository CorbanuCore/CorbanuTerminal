...
- [ ] `acct-chat-impl-03` implemented the partial 19-file Chat candidate, then STOPPED on a frozen native contract counterexample: section 6 expects two requests after `length`, but `session/turn.rs::CompletionFinishReason::Length` returns InvalidRequest without further model work. Intermediate API 10/10 pass; Core/native 24/28 pass (2 LEAK), 4 failures; no final-tree, mutation or receiving qualification. Fable must disposition the length expectation and unresolved spawn/cross-wire fixtures before redispatch; preserve existing continuation behavior. [Third-attempt evidence](../../../../qa/portfolio/agent-cost-accounting/pf-60-s02/chat-dispatch-increment.md#third-attempt--acct-chat-impl-03-2026-09-15-stopped-on-native-contract). Collection OFF; S02 open, S03 dependent.

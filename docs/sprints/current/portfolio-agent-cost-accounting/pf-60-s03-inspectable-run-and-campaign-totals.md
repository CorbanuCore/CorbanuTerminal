...
- [ ] **Blocked on the owner decision.** The inspector's data paths cannot be functionally qualified because collection has no activation outside tests; raised as decision `accounting-collection-activation-20260916`. All four findings from the code-blind guest run are now closed. Everything else this sprint can prove without that decision has been proven.

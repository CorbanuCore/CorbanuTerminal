...
- [ ] **Three findings disclosed from S03-C review, none blocking; one scoped correction.** The coverage line calls a bucket whole when only the aggregate is retained and raw detail is partly expired, so that sentence is an aggregate-retention claim while the next one says detail is checked separately; no number is fabricated and the bucket still renders unavailable. The range overview total carries less disclosure than the same total in the day view, which attaches the unresolved-ancestry caveat. No store-level test exercises a bucket spanning two UTC days, and review named two surviving mutants there. Functional qualification of the range controls also remains open.
- [ ] Qualify retained sub-day precision and unsupported older precision through real keys, including concrete whole-UTC-day alternatives without rounding, prorating or retention extension.

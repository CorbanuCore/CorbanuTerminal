...
- [ ] **Blocked on the owner decision, plus one finding still open.** The inspector's data paths cannot be qualified because collection has no activation outside tests, raised as decision `accounting-collection-activation-20260916`. Separately, the hour caveat is still day-oriented when a bucket contains an unavailable day, which is the path the guest kept hitting, and its test asserts inspector internals rather than a rendered screen - which is how a user-visible fix went half-done without failing.

...
- [x] 2026-09-16 `acct-inspect-fix-10` corrects the two follow-up wording findings: coverage names the requested/aggregate-retention/snapshot intersection, and ancestry figures explicitly say Range or Bucket. The [revision receipt](../../../../qa/portfolio/agent-cost-accounting/pf-60-s03/acct-inspect-fix-10-return.md) records failing-before regressions and final verification. Accounting calculations remain unchanged; receiving review and functional qualification of the range controls remain open.
...
- [ ] **Functional qualification of the inspector, with one known residue folded in.** Qualify retained sub-day precision and the refusal of unsupported older precision through real keys, including the concrete whole-UTC-day bounds offered instead. Four review rounds on the rendering wording each found smaller residue than the last; what is left is one unscoped attempt count on the bucket unknown-parent child page. A human reading the actual screens will find anything remaining far more reliably than a fifth diff review would, so it is qualified here rather than polished again.

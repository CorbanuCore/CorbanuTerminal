...
- [ ] Disclosed from review and not fixed: `RetainedDay::NeedsMaintenance` outranks a stale raw day, so that combined state renders checkpoint-lag wording instead of stale-contribution wording. No number is emitted in either branch, so no false total can appear; the precedence is currently unexercised by any test. Next scoped correction.

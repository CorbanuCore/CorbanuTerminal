...
- [ ] **Two wording findings from the correction's own review, one more pass.** Renaming the coverage line to name aggregate retention over-attributes clips that actually came from the requested window or from snapshot freshness, so a bucket limited by neither now says retention limited it. And the range-wide ancestry counts are spliced into every bucket header beside that bucket's own differing count, so two unreconciled figures sit on one page, weakening the disclosure the correction was meant to strengthen. Neither is a wrong number. Functional qualification of the range controls also remains open.

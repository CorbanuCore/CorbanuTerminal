...
- [ ] **Product finding exposed by the qualification, not by the harness:** increment D can apply a listener hold but cannot lift it across a restart (`restart_allowed` hard-requires a non-stopped phase), which is why cases D07 and D08 could not be built. A supervisor that can enter a held state and not leave it is a real gap in work already received at `aba3707b0`, and it needs its own fix rather than a harness workaround.

...
- [x] **CLI recovery, input, retention and restart fences scoped, 2026-09-16, manager decision.** A CLI acquires the queue lock with a bounded wait and names the holder rather than inheriting `flock`'s blocking-forever; a record whose delivery is genuinely unknown after a crash is reported uncertain and never auto-retried, because retrying an uncertain send is how a duplicate is posted; recovery is read-only plus an explicit named `retry`, with receipt deletion not offered at all rather than warned about; credentials stay file-only and no free-form payload is accepted, since events are built from validated run records; outbox records are retained until an explicit owner action, deliberately unlike accounting retention, because an outbox record is the only local evidence that a post happened. [Scope](../../../../qa/initiative-control/pf-80-s01/cli-fences-20260916.md).

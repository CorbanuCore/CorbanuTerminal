...
## Remaining
- [ ] Product fixes for those four defects are running, each with a failing test written first and mutation proof, and with an explicit instruction not to weaken any existing fence to make a case pass. The harness half is running separately: `D04` conflates a live process with an active consumer, `D06` never established unreadability with `chmod(0)`, `D09` and `D10` have fixture and assertion overreach, and the `D14` oracle crashed instead of reporting a missing field. Narrowing must not erase the genuine `D09` and `D10` findings.

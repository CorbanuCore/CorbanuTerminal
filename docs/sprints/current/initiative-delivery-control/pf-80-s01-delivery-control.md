...
- [ ] Reconcile historical PF-76 reports/queued events with PF-80 explicitly; no silent replay, deletion or automatic flushing of the historical batch. **Inventory complete 2026-09-16: nothing is held** - no queued event exists in live state and the only such report is a synthetic fixture, so there is nothing to migrate. Remaining half: the hold keys on the shared identifier, which also blocks the live security sprint PF-76-S01 from ever reporting; discriminate on `source_namespace` instead. [Inventory](../../../../qa/initiative-control/pf-80-s01/pf76-inventory-20260916.md).

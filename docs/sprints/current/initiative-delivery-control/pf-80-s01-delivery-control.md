...
## Remaining
- [ ] **Functional harness is NOT qualifiable as built; independent review returned P1 blocking.** The 13 built cases are really 6 executable-but-uncontrolled plus 7 controlled-but-unexecutable: `run.py:285` diverts the seven controlled cases to `extended_runner`, which returns a fixed verdict, while the six executable cases have no broken-variant control at all. There is no positive control and zero real executions, so a pass would mislead. The 27 rejected variants are 12 distinct deltas, since D03 and D09 share one blanket mutation. The harness itself honestly reports `execution_readiness: blocked` and claims nothing. Next round must make the two halves meet.

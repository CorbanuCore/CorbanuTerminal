...
- [ ] Integrate the constructed rolling service substage at `6bdc84195`: typed broker/Vault/PF-41 composition, trusted socket inputs, default denial and synthetic Linux subprocess lifecycle are implemented and remotely tested (1 default + 6 subprocess + 338 affected + 6 focused Core tests). EINTR handling is implemented and exercised with native signals. Review 5 Astra High passed clean (helper exit 0); fresh Fable coverage awaits review-budget clarification (5/5 consumed). See `qa/security-levels/sprints/PF-27-S04/service-construction-evidence.md`. No production integrity anchor, principal creation, installation, ownership/ACL migration or existing Vault transfer is claimed. PF-20 owns the missing native durable anchor; provider streaming remains a separate gate.

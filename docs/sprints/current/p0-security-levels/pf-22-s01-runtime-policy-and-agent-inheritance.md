...
depends_on: "PF-19-S01, PF-20-S01, PF-21-S01, PF-41-S03"
created: 2026-08-24
updated: 2026-08-28
---
...
- Planned seam register/checker: `qa/security-levels/upstream-seams.json`, `scripts/security-upstream-seams-check`, `scripts/tests/test_security_upstream_seams.py`; Core integration tests for actual hook coverage.
...
- [ ] Populate the Codex integration-seam register required by the plan: exact upstream symbols/revision, Corbanu module, named owner, policy/provenance contract and executable regression command for each ingress/egress/child/persistence hook.
- [ ] Consume the PF-41-S03 event/recovery contract. Test synthetic new ingress/egress hooks and upstream merge conflicts: unknown unregistered paths cannot bypass policy; rerun seam, inheritance, persistence and independent Permissive comparisons after relevant merges.
...
- [ ] Seam register check and self-tests: `python3 scripts/security-upstream-seams-check --manifest qa/security-levels/upstream-seams.json` and `python3 -m unittest discover -s scripts/tests -p 'test_security_upstream_seams.py'`; missing hook/owner/commit/command/evidence must fail.

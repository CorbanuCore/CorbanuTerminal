...
- [x] Read and independently evaluated every row in the 82-record `canary-r3` human packet: accepted 27 as proposed, relabeled 22, rejected 33, and quarantined R3 with record-ID-bound private decisions plus a safe aggregate reason taxonomy.
- [x] Implemented schema-3 prompt and lint corrections for actual-artifact voice, label/family compatibility, suspicious/hostile boundaries, operative family and foreign-language signals, real hard-negative lookalikes, human-only phishing, self-announcement, cross-segment semantics, completion, and disjoint-audit strata.
- [x] Ran and audited replacement canary iterations through `canary-r4e`. R4e retained 571/1,000 candidates after 426 deterministic rejection entries; all 51 human-queue rows were reviewed, with 39 accepted as proposed, one relabeled and 11 rejected. The round is quarantined rather than admitted. The failures are semantic generator quality, not RTX throughput, power, RAM, VRAM, or thermals.

## Remaining

- [ ] Select a stronger or further-tuned generation strategy and pass a fresh 1,000-candidate canary through deterministic gates, complete human content audit, the disjoint Opus audit, and integration-owner acceptance. R3 and R4e are quarantined and must not be adjudicated or reused.

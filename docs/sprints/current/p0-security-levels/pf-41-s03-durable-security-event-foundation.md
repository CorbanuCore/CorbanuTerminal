...
- [x] Added validated recovery tail caching, a live unresolved-intent fence and explicit exact-event operator reconciliation for one-record ambiguous commits; restart/root changes invalidate the cache, reconciliation grants no permit and an unresolved intent must receive a completed/unknown terminal result before another dispatch.
...
- [x] Final Rust formatting, full-workspace-deny Clippy, 36 unit/fault tests, one public integration test, three fixture tests, 23 governance checker tests and both live governance checkers pass; exact commands and artifact hashes are recorded in evidence.
...
- [ ] Complete the fresh final read-only Claude Opus 5 Plan Max review in the TMUX/Corbanu harness; all earlier findings are remediated and retested.

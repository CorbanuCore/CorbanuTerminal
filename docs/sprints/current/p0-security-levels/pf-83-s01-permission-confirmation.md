...
## Remaining
- [ ] **Case execution needs machinery that was never built.** The run reached `AssertionError: preflight only; cases prohibited`, missing `launch_package`, `pty_action` and `close_owned` handlers and an independent actor runtime. The harness has only ever been a preflight tool. All 380 verdicts remain blocked and 30/175 is unchanged. This is a build, not a repair, and it needs an explicit decision on whether it is worth it before starting.

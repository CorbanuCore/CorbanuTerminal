...
## Remaining
- [ ] **PF83-DEF-015: a declined command is reported as having run.** After a decline the history shows both 'You canceled the request to run ...' and 'Ran ... (no output)', while the fixture observation proves nothing executed, so a user would reasonably believe a command they refused was executed. Found code-blind during F01/F02; on a different axis from those cases' outcomes, so it overturns neither PASS. Beside it: idle application wording carries active-turn instructions, and /status separately reports security protection as unverified.

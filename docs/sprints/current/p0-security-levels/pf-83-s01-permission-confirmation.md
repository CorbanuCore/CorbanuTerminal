...
- [ ] **Per-run OS enforcement design accepted** (`a99394f7`): a `sandbox-exec` profile with literal text, applied before bridge startup with inherited child restrictions and separate trusted witnesses. It maps roughly fourteen controls to `EPERM` and **names the rest as unprovable with specific reasons** rather than promising 175: Mach and OSStatus error domains that are not POSIX `-1`/errno, missing fixtures, and controls needing dispatcher enforcement. Estimated increment 620 to 770 lines across three new and three existing files. [Design](../../../../qa/initiative-control/management-bootstrap/pf83-sandbox-design-20260915.md).

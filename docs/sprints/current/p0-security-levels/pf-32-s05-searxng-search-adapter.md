---
sprint_id: "PF-32-S05"
title: "SearXNG brokered search adapter"
status: draft
...
# PF-32-S05 — SearXNG brokered search adapter

## Execution mandate

- Deliver: SearXNG search is usable through the common screened contract without raw credential or history disclosure.
...
- Acceptance advanced: SearXNG search is usable through the common screened contract without raw credential or history disclosure.
- Sources and archive disposition: [PF-32 reconciliation](../../../plans/security-source-reconciliation.md#pf-32).

## Code boundaries

- OpenClaw adoption reference: [OC-9](../../../plans/openclaw-source-review-2026-08-28.md#oc-9) at `13adff02ca3897768d80d2bca18f5acf08c55d91`; see the review for named functions, callers, tests and limits. Reference tests are not candidate evidence.

- Existing/foundation: PF-32 provider registry and normalized results; PF-33 destination policy.
- Planned: codex-rs/ext/web-search/src/broker/searxng.rs; codex-rs/ext/web-search/tests/searxng.rs.
- Tests: planned colocated Rust test modules prefixed `pf_32_s05`; fixtures use synthetic secrets and fake services only.
...
- [ ] Keep self-hosted SearXNG's human-approved exact endpoint exception separate from public-fetch SSRF policy; test redirects, changed ports, DNS changes and proxies without inheriting a blanket private-network allow.

- [ ] Pin the reviewed SearXNG API schema and documentation at implementation; declare exact endpoint/auth/search-only capabilities, pagination, limits and unsupported commands.
- [ ] Require a human-configured instance and explicit privacy/trust disclosure; private service access uses the exact PF-33 adapter exception, never arbitrary private URL fetch.
...
- [ ] Add named `pf_32_s05` regression tests; update affected Cargo/Bazel/lock/schema edges together without broadening this feature.

## Verification

- [ ] Run `cd codex-rs && just fix -p <affected-crate>` for each listed crate, then `just fmt`; inspect the final diff.
- [ ] Focused: `cd codex-rs && just test -p codex-web-search-extension pf_32_s05`; confirm tests actually ran.
...
- [ ] Implementation commit and final-tree outputs under `qa/security-levels/sprints/PF-32-S05/`.

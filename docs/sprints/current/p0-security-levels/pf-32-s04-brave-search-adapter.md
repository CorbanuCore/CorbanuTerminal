---
sprint_id: "PF-32-S04"
title: "Brave brokered search adapter"
status: draft
...
# PF-32-S04 — Brave brokered search adapter

## Execution mandate

- Deliver: Brave search is usable through the common screened contract without raw credential or history disclosure.
...
- Acceptance advanced: Brave search is usable through the common screened contract without raw credential or history disclosure.
...
- Planned: codex-rs/ext/web-search/src/broker/brave.rs; codex-rs/ext/web-search/tests/brave.rs.
- Tests: planned colocated Rust test modules prefixed `pf_32_s04`; fixtures use synthetic secrets and fake services only.
...
- [ ] Record the Brave adapter's own pinned API contract and tests; carry brokered auth and common screening through results, metadata, errors and failover. The reference review does not qualify this provider integration.

- [ ] Pin the reviewed Brave API schema and documentation at implementation; declare exact endpoint/auth/search-only capabilities, pagination, limits and unsupported commands.
...
- [ ] Add named `pf_32_s04` regression tests; update affected Cargo/Bazel/lock/schema edges together without broadening this feature.

## Verification

- [ ] Run `cd codex-rs && just fix -p <affected-crate>` for each listed crate, then `just fmt`; inspect the final diff.
- [ ] Focused: `cd codex-rs && just test -p codex-web-search-extension pf_32_s04`; confirm tests actually ran.
...
- [ ] Implementation commit and final-tree outputs under `qa/security-levels/sprints/PF-32-S04/`.

...
interface PeriodRow {
  ...
  monthly_used_tokens: string;
  monthly_reserved_tokens: string;
}
interface WeeklyRow {
  sequence: string;
  starts_at: Date;
...
export class PostgresGatewayStore implements GatewayStore {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      await advisoryLock(client, "pfterminal:ambient-gateway:schema");
      await client.query(`
      ...
      CREATE TABLE IF NOT EXISTS ambient_accounting_wallets (
        wallet_address TEXT PRIMARY KEY,
        classification TEXT NOT NULL CHECK (classification IN ('customer', 'internal')),
  ...
  async recordSettlement(settlement: Settlement): Promise<SubscriptionPeriod> {
    ...
    try {
      ...
      if (existing) {
        if (
          existing.wallet_address !==
            normalizeAddress(settlement.walletAddress) ||
          existing.plan_id !== settlement.planId
        ) {
          throw new Error(
      ...
      const startsAt =
        latest.rows[0]?.ends_at && latest.rows[0].ends_at > settlement.settledAt
          ? latest.rows[0].ends_at
      ...
      await client.query("COMMIT");
      return rowToPeriod(
        required(inserted.rows[0], "settlement insert returned no period"),
      );
    } catch (error) {
      await client.query("ROLLBACK");
      throw error;
  ...
  async createApiKey(
    ...
    if (!(await this.activePeriod(wallet, now)))
      throw new Error(
        "an active subscription is required to create an API key",
      );
  ...
  async accountForApiKey(
    ...
    try {
      ...
      return {
        walletAddress: period.walletAddress,
        ...snapshot(period, weekly),
        queuedPeriods: queued.rows.map(rowToPeriod),
      };
    } finally {
      client.release();
    }
  ...
  async listApiKeys(walletAddress: string): Promise<ApiKeySummary[]> {
    const result = await this.pool.query<{
      ...
      "SELECT id,display_prefix,created_at,revoked_at,last_used_at FROM ambient_api_keys WHERE wallet_address=$1 ORDER BY created_at",
  ...
  async reserveApiKeyUsage(
    ...
    try {
      ...
      const prior = await findReservation(
        client,
        active.period.wallet_address,
        requestId,
      );
      if (prior) {
        if (
          prior.model !== model ||
          units(prior.reserved_tokens_request) !== tokens
        ) {
          throw new Error(
      ...
      const weeklyRemaining =
        units(weekly.limit_tokens) -
        units(weekly.used_tokens) -
        units(weekly.reserved_tokens);
      const monthlyRemaining =
        period.monthlyLimitTokens -
        period.monthlyUsedTokens -
        period.monthlyReservedTokens;
      if (weeklyRemaining < tokens) {
        await client.query("ROLLBACK");
        return {
          kind: "limit",
          window: "weekly",
          limitTokens: units(weekly.limit_tokens),
          usedTokens: units(weekly.used_tokens),
          reservedTokens: units(weekly.reserved_tokens),
          remainingTokens: Math.max(0, weeklyRemaining),
          resetsAt: new Date(weekly.ends_at),
        };
      }
      ...
      const id = randomUUID();
      await client.query(
        "UPDATE ambient_subscription_periods SET monthly_reserved_tokens=monthly_reserved_tokens+$2 WHERE transaction=$1",
        [period.transaction, tokens],
      );
      await client.query(
        "UPDATE ambient_weekly_windows SET reserved_tokens=reserved_tokens+$3 WHERE period_transaction=$1 AND sequence=$2",
        [period.transaction, weekly.sequence, tokens],
      );
      await client.query(
        "UPDATE ambient_api_keys SET last_used_at=$2 WHERE id=$1",
        [active.keyId, now],
      );
      await client.query(
      ...
      return {
        kind: "authorized",
        reservation: reservationFromRow(
          required(saved, "reservation insert failed"),
        ),
      };
    } catch (error) {
      await client.query("ROLLBACK");
  ...
  async settleApiKeyUsage(
    ...
    try {
      ...
      const usageSource =
        usage?.source ?? (ambiguous ? "reservation" : undefined);
      await client.query(
      ...
      return reservationFromRow(
        required(saved, "settled reservation disappeared"),
      );
    } catch (error) {
      await client.query("ROLLBACK");
      throw error;
  ...
  async hasUsedNonce(nonce: string): Promise<boolean> {
    const result = await this.pool.query(
      "INSERT INTO ambient_used_siwx_nonces(nonce) VALUES($1) ON CONFLICT(nonce) DO NOTHING RETURNING nonce",
      [nonce],
    );
...
const PERIOD_COLUMNS =
  "transaction,wallet_address,plan_id,plan_revision,starts_at,ends_at,monthly_limit_tokens,monthly_used_tokens,monthly_reserved_tokens";
function prefixColumns(prefix: string): string {
  return PERIOD_COLUMNS.split(",")
    .map((column) => `${prefix}.${column}`)
    .join(",");
}

async function activeForHash(
  client: PoolClient,
  hash: string,
  now: Date,
  lock: boolean,
  ...
  const result = await client.query<PeriodRow & { key_id: string }>(
    `SELECT ${prefixColumns("p")},k.id key_id FROM ambient_api_keys k
     ...
     ORDER BY p.starts_at LIMIT 1${lock ? " FOR UPDATE OF p,k" : ""}`,
    [hash, now],
  );
...
async function ensureWeeklyWindow(
  ...
  const sequence = Math.max(
    0,
    Math.floor((now.getTime() - period.starts_at.getTime()) / WEEK_MS),
  );
  const startsAt = new Date(period.starts_at.getTime() + sequence * WEEK_MS);
  const endsAt = new Date(
    Math.min(startsAt.getTime() + WEEK_MS, period.ends_at.getTime()),
  );
  const planId = required(
    parsePlanId(period.plan_id),
    "unsupported stored plan",
  );
  await client.query(
...
async function findTransaction(
  ...
  return (
    await client.query<PeriodRow>(
      `SELECT ${PERIOD_COLUMNS} FROM ambient_subscription_periods WHERE transaction=$1`,
      [transaction],
    )
  ).rows[0];
}
...
async function findReservation(
  ...
  return (
    await client.query<ReservationRow>(
      `${RESERVATION_SELECT} WHERE l.wallet_address=$1 AND l.request_id=$2`,
      [wallet, requestId],
    )
  ).rows[0];
}
async function findReservationById(
  ...
  return (
    await client.query<ReservationRow>(
      `${RESERVATION_SELECT} WHERE l.id=$1${lock ? " FOR UPDATE OF l,p,w" : ""}`,
      [id],
    )
  ).rows[0];
}
...
function rowToPeriod(row: PeriodRow): SubscriptionPeriod {
  const planId = required(
    parsePlanId(row.plan_id),
    `unsupported stored plan: ${row.plan_id}`,
  );
  return {
    transaction: row.transaction,
    walletAddress: row.wallet_address,
    planId,
    planRevision: units(row.plan_revision),
    startsAt: new Date(row.starts_at),
    endsAt: new Date(row.ends_at),
    monthlyLimitTokens: units(row.monthly_limit_tokens),
    monthlyUsedTokens: units(row.monthly_used_tokens),
    monthlyReservedTokens: units(row.monthly_reserved_tokens),
  };
}
function rowToWeekly(row: WeeklyRow): WeeklyUsage {
  return {
    startsAt: new Date(row.starts_at),
    endsAt: new Date(row.ends_at),
    limitTokens: units(row.limit_tokens),
    usedTokens: units(row.used_tokens),
    reservedTokens: units(row.reserved_tokens),
  };
}
function snapshot(
  ...
  const weekly = rowToWeekly(weeklyRow);
  return {
    period,
    weekly,
    monthlyRemainingTokens: Math.max(
      0,
      period.monthlyLimitTokens -
        period.monthlyUsedTokens -
        period.monthlyReservedTokens,
    ),
    weeklyRemainingTokens: Math.max(
      0,
      weekly.limitTokens - weekly.usedTokens - weekly.reservedTokens,
    ),
  };
}
function reservationFromRow(row: ReservationRow): UsageReservation {
  const snap = snapshot(rowToPeriod(row), row);
  const input = nullableUnits(row.input_tokens);
  const output = nullableUnits(row.output_tokens);
  return {
    id: row.id,
    requestId: row.request_id,
    model: row.model,
    reservedTokens: units(row.reserved_tokens_request),
    state: row.state,
    chargedTokens: nullableUnits(row.charged_tokens),
    ...snap,
    actualUsage:
      input === undefined || output === undefined
        ? undefined
        : {
            source: row.usage_source === "estimated" ? "estimated" : "upstream",
            inputTokens: input,
            outputTokens: output,
            cachedInputTokens: nullableUnits(row.cached_input_tokens) ?? 0,
            reasoningTokens: nullableUnits(row.reasoning_tokens) ?? 0,
...
function units(value: string | number): number {
  ...
  if (!Number.isSafeInteger(parsed) || parsed < 0)
    throw new Error("database contains invalid token units");
  return parsed;
}
function nullableUnits(value: string | null): number | undefined {
  return value === null ? undefined : units(value);
}
